...
import getActiveRegistrations from '@salesforce/apex/AgentGovSelector.getActiveRegistrations';
...
export default class AgentGovDashboard extends LightningElement {
    ...
    async loadData() {
        this.isLoading = true;
        try {
            const [allAgents, budgets, conflicts] = await Promise.all([
                getAllRegistrations(),
                getAllTodaysBudgets(),
            ...
            this.budgets = (budgets || []).map(b => {
                const apiPercent = b.API_Calls_Allocated__c > 0
                    ? Math.round((b.API_Calls_Consumed__c / b.API_Calls_Allocated__c) * 100)
                    : 0;
                const soqlPercent = b.SOQL_Queries_Allocated__c > 0
                    ? Math.round((b.SOQL_Queries_Consumed__c / b.SOQL_Queries_Allocated__c) * 100)
                    : 0;
                const dmlPercent = b.DML_Operations_Allocated__c > 0
                    ? Math.round((b.DML_Operations_Consumed__c / b.DML_Operations_Allocated__c) * 100)
                    : 0;
    ...
    getStatusClass(status) {
        const map = {
            Normal: 'slds-theme_success',
            Warning: 'slds-theme_warning',
            Throttled: 'slds-theme_warning',
            Blocked: 'slds-theme_error',
